...
Z3ASTHandle Z3Builder::buildArray(const char *name, unsigned indexWidth,
                                  unsigned valueWidth) {
  Z3SortHandle domainSort = Z3SortHandle(Z3_mk_int_sort(ctx), ctx);//getBvSort(indexWidth);
  Z3SortHandle rangeSort = Z3SortHandle(Z3_mk_int_sort(ctx), ctx);//getBvSort(valueWidth);
...
Z3ASTHandle Z3Builder::getInitialRead(const Array *root, unsigned index) {
  Z3SortHandle t = Z3SortHandle(Z3_mk_int_sort(ctx), ctx); 
  return readExpr(getInitialArray(root), Z3ASTHandle(Z3_mk_unsigned_int64(ctx, index, t), ctx));
}
...
Z3ASTHandle Z3Builder::constructActual(ref<Expr> e, int *width_out) {
  ...
  switch (e->getKind()) {
  // TODO: isnan, isinf, sqrt, abs need to be handled in new representation of FP 
  case Expr::Constant: {
    ...
    *width_out = CE->getWidth();
    ...
    int j=0;
    llvm::SmallVector<char,20>::iterator i;
    ...
    str[j]=0;
    Z3_sort r=Z3_mk_real_sort(ctx);
    return Z3ASTHandle(Z3_mk_numeral(ctx, (const char*)str, r),ctx);
  ...
  case Expr::ZExt: {
    ...
    *width_out = ce->getWidth();
    if(ce->src->getWidth() == Expr::Bool)  {
      Z3SortHandle t1 = Z3SortHandle(Z3_mk_int_sort(ctx), ctx); 
      Z3SortHandle t2 = Z3SortHandle(Z3_mk_int_sort(ctx), ctx);      
      return iteExpr(src, 
            Z3ASTHandle(Z3_mk_int64(ctx, 1, t1), ctx), 
            Z3ASTHandle(Z3_mk_int64(ctx, 0, t2), ctx));
    }
    return src;
    /*
    if (srcWidth == 1) {
...
// In our scenario only FAdd, FSub, FMul, FDiv is used among the instructions below so at least they need to be handled.
  case Expr::FOEq: {
    EqExpr *ee = cast<EqExpr>(e);
    Z3ASTHandle left = construct(ee->left, width_out);
    Z3ASTHandle right = construct(ee->right, width_out);
  ...
  case Expr::FOGt: {
    ...
    *width_out = 1;
    return Z3ASTHandle(Z3_mk_gt(ctx, left, right), ctx);
    /*
  ...
  */
  ...
  case Expr::FOGe: {
    ...
    *width_out = 1;
    return Z3ASTHandle(Z3_mk_ge(ctx, left, right), ctx);
  ...
  */
  ...
  case Expr::FAdd: {
    ...
    return Z3ASTHandle(Z3_mk_fpa_add(ctx,                                 getRoundingModeSort(fadd->roundingMode),
                                     left, right),
                       ctx);*/
  }
  ...
  case Expr::FSub: {
    ...
    return Z3ASTHandle(Z3_mk_fpa_sub(ctx,                             getRoundingModeSort(fsub->roundingMode),
                                     left, right),
                       ctx);*/
  }
